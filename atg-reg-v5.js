...
function detectGeo() {
  return fetch('https://ipapi.co/json/?fields=country_code,country_name,currency')
    ...
    .then(function(d) {
      ...
      GEO.countryName  = d.country_name || GEO.country;
...
function goToPayment() {
  ...
  fd = {
    ...
    country:      g('country') || GEO.countryName || GEO.country || 'IN',
...
function updateAmountDisplay() {
  var sym = GEO.isIndia ? 'Rs.' : '$';
  if(el('totalAmt'))    el('totalAmt').textContent    = sym + fmt(finalAmt);
  if(el('baseFeeDisp')) el('baseFeeDisp').textContent = sym + fmt(CFG.baseAmount);
...
function selectGW(gw) {
  ...
  if(btn) {
    ...
    var sym2 = GEO.isIndia ? 'Rs.' : '$';
    btn.textContent = 'Pay ' + sym2 + fmt(finalAmt) + ' via ' + info.name;
  }
...
(function init() {
  ...
  // Detect user location silently — no loader shown on Step 1
...
function applyGeoPricing(geo) {
  // Silently set pricing — no banner shown on Step 1
  CFG.baseAmount = geo.amount;
  finalAmt       = geo.amount;
  if (!geo.isIndia) window._intlUser = true;

  // Auto-fill the country field (read-only)
  var countryInp = el('country');
  if (countryInp) {
    countryInp.value = geo.countryName || geo.country || (geo.isIndia ? 'India' : geo.country);
  }
}

// switchToIntl kept for debug/console use — does NOT show step-1 banner
